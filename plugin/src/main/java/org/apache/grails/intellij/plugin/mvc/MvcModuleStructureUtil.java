...
public final class MvcModuleStructureUtil {
  ...
  public static final @NonNls String PLUGINS_DIRECTORY = "plugins";
  public static final @NonNls String APPLICATION_PROPERTIES = "application.properties";

  private MvcModuleStructureUtil() {
  }

  public static @Nullable ContentEntry findContentEntry(ModuleRootModel rootModel, VirtualFile root) {
  ...
  private static @Nullable Consumer<ModifiableRootModel> addSourceRootsAndLibDirectory(final @NotNull VirtualFile root,
  ...
  public static @Nullable Consumer<ModifiableRootModel> addJarDirectory(VirtualFile root, Module module, final String libName) {
  ...
  private static OrderEntry @Nullable [] moveupLibrariesFromMavenPlugin(ModuleRootModel moduleRootModel) {
    ...
    for (int i = 0; i < orderEntries.length; i++) {
      if (orderEntries[i] instanceof LibraryOrderEntry libraryEntry) {
  ...
  private static @Nullable Library extractNonModuleLibraries(List<? super Library> result,
  ...
  public static void syncAuxModuleSdk(@NotNull Module appModule, @NotNull Module pluginsModule, final @NotNull GrailsFramework framework) {
  ...
  public static @Nullable PropertiesFile findApplicationProperties(@NotNull Module module, GrailsFramework framework) {
  ...
  public static @NotNull Module createAuxiliaryModule(@NotNull Module appModule, final String moduleName, final GrailsFramework framework) {
    ...
    moduleModel.newModule(moduleFilePath, JavaModuleType.getModuleType().getId());
    moduleModel.commit();
  ...
  public static void copySdkAndLibraries(ModuleRootModel from, ModifiableRootModel to, @NotNull GrailsFramework framework) {
    copySdk(from, to);
    copyUserLibraries(from, to, framework);
  }

  public static void copyUserLibraries(ModuleRootModel from, ModifiableRootModel to, @NotNull GrailsFramework framework) {
  ...
  public static void updateAuxModuleStructure(Module auxModule, Collection<VirtualFile> pluginDirs, GrailsFramework framework) {
  ...
  public static void updateGlobalPluginModule(@NotNull Project project, @NotNull GrailsFramework framework) {
    ...
    for (VirtualFile virtualFile : map.keySet()) {
      ...
      for (Module appModule : map.get(virtualFile)) {
        ...
        Module commonPluginsModule = GrailsFramework.findCommonPluginsModule(appModule);
  ...
  public static @Nullable Module updateAuxiliaryPluginsModuleRoots(Module appModule, GrailsFramework framework) {
    Module commonPluginsModule = GrailsFramework.findCommonPluginsModule(appModule);
    ...
    for (VirtualFile pluginRoot : framework.getCommonPluginRoots(appModule, true)) {
      if (checkValidity(pluginRoot)) {
        if (globalPluginsDir == null || !VfsUtilCore.isAncestor(globalPluginsDir, pluginRoot, true)) {
    ...
    if (commonPluginsModule == null) {
      commonPluginsModule = createAuxiliaryModule(appModule, GrailsFramework.getCommonPluginsModuleName(appModule), framework);
    }
  ...
  public static @Nullable VirtualFile refreshAndFind(@Nullable File file) {
    return findFile(file, true);
  }

  public static @Nullable VirtualFile findFile(@Nullable File file, boolean refresh) {
